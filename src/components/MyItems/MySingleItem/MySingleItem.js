  ...
  return (
    ...
    <div className="card card-side bg-base-100 shadow-xl w-full max-w-[500px] h-[260px] relative">
      ...
      <div className=" w-[58%] ml-3">
        <h2 className="card-title underline text-2xl font-bold text-blue-700">{name}</h2>
        <div className="mt-2">
          <p>
            <span className="font-semibold text-red-800 text-lg">Supplier:</span>
            {supplierName}
          </p>
          <p className="mt-2">
            <span className="font-semibold text-red-800 text-lg">Quantity:</span>
            {quantity}
          </p>
          <p className="mt-2">
    ...
    <Modal
      ...
      >
        <div>
          ...
          <div className="mt-4">
            <p className="text-center font-semibold">Write 'CONFIRM' to delete the vehicle</p>
          <input
          ...
          <button
            className={`px-4 py-2 text-lg font-semibold text-white rounded-md ${conDelete==='CONFIRM'?'bg-red-700':'bg-red-700 bg-opacity-40'}`}
            onClick={() => confirmDelete(_id)}
            disabled={conDelete === "CONFIRM" ? false : true}
          >
            DELETE
          </button>
          <button onClick={closeModal} className="ml-2 text-white font-semibold px-4 py-2 text-lg bg-green-700 rounded-md">CANCEL</button>
          </div>
        </div>
      </Modal>
    </>
